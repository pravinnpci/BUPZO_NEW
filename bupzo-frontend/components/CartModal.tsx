...
type CartModalProps = {
  ...
  user: any;
  onCheckout: (walletAmountUsed: number, shippingCost: number, shippingPartner: string) => void;
  promoCode: string;
...
export default function CartModal({
  ...
  const maxWalletUsable = user?.wallet_balance ? Math.min(totalAfterDiscount + currentShippingCost, parseFloat(user.wallet_balance)) : 0;
  const walletAmountUsed = useWallet ? maxWalletUsable : 0;
  const finalTotal = totalAfterDiscount + currentShippingCost - walletAmountUsed;
          ...
          <div className="bg-white border-t border-gray-200 p-6 space-y-4">
            ...
            <div className="space-y-2 text-sm font-medium text-gray-600 border-t border-gray-100 pt-4">
              ...
              <div className="flex justify-between text-lg font-extrabold text-[#232f3e] border-t border-gray-200 pt-2 mt-3">
                <span>Total</span>
                <span>₹{(totalAfterDiscount + currentShippingCost).toLocaleString()}</span>
              </div>
            </div>

            {user && user.wallet_balance > 0 && (
              <div className="bg-blue-50 p-3 rounded border border-blue-200 text-sm flex justify-between items-center">
                <div>
                  <h4 className="font-bold text-blue-800 flex items-center gap-1"><span className="material-symbols-outlined text-[16px]">account_balance_wallet</span> Wallet Balance</h4>
                  <p className="text-blue-600">Available: ₹{parseFloat(user.wallet_balance).toLocaleString()}</p>
                </div>
            ...
            {useWallet && (
               <div className="flex justify-between text-lg font-extrabold text-[#232f3e] bg-yellow-50 p-2 rounded">
            ...
            {user && (
              <div className="bg-gray-50 p-3 rounded border border-gray-200 text-sm">
                ...
                {savedAddresses.length > 0 && (
                  <select 
                    className="w-full border border-gray-300 rounded p-2 mb-2 outline-none text-xs"
                    onChange={(e) => {
                      if (e.target.value) {
                ...
                <textarea 
                  ...
                  className="w-full border border-gray-300 rounded p-2 outline-none text-xs text-gray-700"
                  rows={3}
                />
                <button 
                  onClick={handleSaveAddress}
                  className="mt-2 text-xs bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-1 rounded"
                >
            ...
            <button 
              onClick={() => {
                ...
                onClose();
                onCheckout(walletAmountUsed, currentShippingCost, selectedShipping);
              }}
