const fs = require('fs');
let content = fs.readFileSync('bupzo-frontend/components/AuthModal.tsx', 'utf8');

// Add showPassword state
content = content.replace(
    '  const [password, setPassword] = useState(\'\');',
    '  const [password, setPassword] = useState(\'\');\n  const [showPassword, setShowPassword] = useState(false);'
);

// Fix z-index
content = content.replace(
    'className="fixed inset-0 z-50 flex flex-col items-center justify-center',
    'className="fixed inset-0 z-[9999] flex flex-col items-center justify-center'
);

// Fix password input and toggle
content = content.replace(
    '<input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 outline-none focus:border-indigo-600 transition-colors bg-gray-50 focus:bg-white" placeholder="Enter Password" />\n                      <button className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">\n                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>\n                      </button>',
    '<input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300 outline-none focus:border-indigo-600 transition-colors bg-gray-50 focus:bg-white" placeholder="Enter Password" />\n                      <button onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">\n                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={showPassword ? "M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" : "M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"} /></svg>\n                      </button>'
);

// Show Google sign in on register mode as well
content = content.replace(
    "{mode === 'login' && (\n                <div className=\"mt-6 pt-6 border-t border-gray-100 flex justify-center\">\n                  <GoogleLogin",
    "{(mode === 'login' || mode === 'register') && (\n                <div className=\"mt-6 pt-6 border-t border-gray-100 flex justify-center\">\n                  <GoogleLogin"
);

fs.writeFileSync('bupzo-frontend/components/AuthModal.tsx', content);
