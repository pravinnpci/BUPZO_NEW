...
const sellers_list_endpoint = `@app.get("/api/sellers/", response_model=List[SellerResponse])`;
const seller_detail_endpoint = `@app.get("/api/sellers/{seller_id}", response_model=SellerResponse)
...
const product_detail_endpoint = `@app.get("/api/products/{product_id}", response_model=ProductResponse)`;
const product_stats_endpoint = `@app.get("/api/products/{product_id}/stats")
...
content = content.replace(
    `        INSERT INTO wallet_transactions (id, user_id, type, amount, status, description, created_at)
        ...
        """,
        uuid4(), user_id_uuid, 'CREDIT', req.amount, 'COMPLETED', 'Wallet Top Up via UI'`,
