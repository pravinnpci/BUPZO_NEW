...
// Initialize MCP tools endpoints
...
app.post('/api/get_product_recommendations', async (req, res) => {
  try {
    ...
    // Get product IDs from views
    const productIds = viewResult.rows.map(row => row.product_id);

    // Get similar products (simplified - in a real app, this would use a more sophisticated algorithm)
    let query = `
...
app.post('/api/get_spin_win_reward', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      // Generate a random reward amount between 10 and 100
