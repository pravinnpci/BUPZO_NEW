...
CREATE TABLE orders (
    ...
    shipping_partner TEXT, -- e.g., Shiprocket, NimbusPost, Delhivery
    payment_gateway TEXT, -- e.g., Razorpay, Cashfree, PhonePe
    trust_donation_amount DECIMAL(10, 2) DEFAULT 0.00, -- Optional donation
    currency TEXT DEFAULT 'INR',
...
CREATE TABLE reviews (
    ...
    comment TEXT,
    image_urls TEXT[], -- S3 links for review images
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- 9. Coupons Table (Updated with min_order_value)
...
CREATE TABLE payment_logs (
    ...
    payment_intent_id TEXT, -- Gateway specific transaction ID
...
CREATE TABLE admin_settings (
    ...
    value TEXT NOT NULL,
    value_type TEXT DEFAULT 'string', -- e.g., 'string', 'number', 'boolean', 'json'
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- This table links products to sellers, allowing multiple sellers for the same product or unique products per seller.
...
CREATE TABLE spin_win_rewards (
    ...
    status TEXT DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'CLAIMED', 'EXPIRED')),
    payout_method TEXT, -- e.g., 'BANK_TRANSFER', 'UPI'
    transaction_id TEXT, -- Gateway specific payout transaction ID
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
